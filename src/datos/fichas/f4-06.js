      ...
      contenido: [
        {
          ...
          modo: "dos",
          filas: [
            { nom: "Homocedasticidad", sub: "El supuesto que rompe la heterocedasticidad.", cols: ["Var(u | X) = σ² constante"] },
            { nom: "Errores robustos de White", sub: "Válidos con cualquier forma de heterocedasticidad.", cols: ["V̂ = (X′X)⁻¹ (Σ ûᵢ²xᵢxᵢ′) (X′X)⁻¹"] },
            { nom: "Ausencia de autocorrelación", sub: "Falla con cualquier dimensión temporal.", cols: ["Cov(uₜ, uₛ) = 0 para t ≠ s"] },
            { nom: "Durbin-Watson", sub: "≈ 2 sin autocorrelación; → 0 positiva fuerte.", cols: ["DW ≈ 2(1 − ρ̂)"] },
            { nom: "Coste de ignorar ρ > 0", sub: "Con ρ = 0,85 los errores se multiplican por 3,5.", cols: ["factor ≈ √((1+ρ)/(1−ρ))"] },
            { nom: "Newey-West", sub: "Robusto a heterocedasticidad y autocorrelación.", cols: ["HAC con ventana de rezagos"] },
            { nom: "Factor de inflación de la varianza", sub: "R²ⱼ de regresar xⱼ sobre las demás.", cols: ["VIFⱼ = 1 ÷ (1 − R²ⱼ)"] },
            { nom: "Efecto sobre el error estándar", sub: "Multiplica la desviación típica del coeficiente.", cols: ["EE(β̂ⱼ) ∝ √VIFⱼ"] },
            { nom: "Firma de la colinealidad", sub: "El modelo predice y no identifica.", cols: ["F conjunto significativo, t individuales no"] },
            { nom: "Regla de oro", sub: "El diagnóstico que no sesga nada.", cols: ["β̂ insesgado; lo que se rompe es Var(β̂)"] },
          ],
        },
      ],
    },
    {
      ...
      contenido: [
        ...
        },
        {
          tipo: "parrafos",
          items: [
            "**Segundo, autocorrelación.** Supongamos ahora una regresión con veinte observaciones trimestrales cuyos residuos, en orden temporal, describen ondas amplias: suben durante cinco trimestres, bajan durante otros cinco y repiten. El estadístico de Durbin-Watson vale **0,303**, muy lejos del 2 que indicaría independencia, y el coeficiente autorregresivo implícito es **ρ ≈ 0,83**. Cada residuo predice el siguiente casi por completo.",
            "El coste es cuantificable: con ρ = 0,85 el factor de corrección de los errores estándar es √((1+ρ)/(1−ρ)) = **3,51**. Es decir, **los errores estándar clásicos serían tres veces y media menores que los correctos**, y un t aparente de 4 sería en realidad de 1,1. Un resultado espectacular se convierte en nada. Y antes de aplicar Newey-West conviene hacerse la pregunta previa: unos residuos con esa persistencia suelen indicar que **al modelo le falta una variable o una dinámica**, no que el error sea caprichoso.",
      ...
      contenido: [
        {
          ...
          items: [
            ...
            { q: "Con ρ = 0,85, ¿cuánto se equivocan los errores estándar clásicos?", a: "Los subestiman por un factor de aproximadamente 3,5, según √((1+ρ)/(1−ρ)). Un t aparente de 4 sería en realidad de 1,1, es decir, no significativo. El motivo es que las observaciones no aportan información independiente: la muestra efectiva es mucho menor que la nominal." },
